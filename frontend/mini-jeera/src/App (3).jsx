...
/* ══════════════════════════════════════════════════════════════
   MEMBERS PAGE  (with Add Member / Invite)
══════════════════════════════════════════════════════════════ */
function MembersPage({ token, selWS, setSelWS, workspaces }) {
  const [members,  setMembers]  = useState([]);
  const [loading,  setLoading]  = useState(false);
  const [modal,    setModal]    = useState(false);
  const [form,     setForm]     = useState({ username: "", role: "DEVELOPER" });
  ...
  async function invite() {
    ...
    try {
      await apiFetch(
        ROUTES.inviteMember(selWS.work_id),
        { method: "POST", body: JSON.stringify({ username: form.username.trim(), role: form.role }) },
        token
      );
      setMsg("✓ Member added successfully!");
      setForm({ username: "", role: "DEVELOPER" });
  ...
  /* ── role badge ── */
  const ROLE_MAP = {
    PROJECT_MANAGER: { bg: "rgba(212,255,0,.13)",   c: "var(--acc)",  label: "Project Manager" },
    DEVELOPER:       { bg: "rgba(68,136,255,.13)",  c: "#5599FF",     label: "Developer"       },
    VIEWER:          { bg: "rgba(128,128,128,.13)", c: "var(--txt2)", label: "Viewer"          },
  };

  function roleBadge(raw) {
    const key = (raw || "VIEWER")
      .replace(/^Roles\./i, "")
      .replace(/\./g, "")
      .toUpperCase();
    const cfg = ROLE_MAP[key] || ROLE_MAP.VIEWER;
    return (
      <span style={{
  ...
  const AVATAR_COLORS = ["var(--acc)", "#5599FF", "#FF8C00", "#3DFF96", "#FF4545", "#CC88FF"];
    ...
    <div className="fi" style={{ padding: 26 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 26, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <p style={{ color: "var(--txt2)", fontSize: 15 }}>
            {selWS
              ? <><strong style={{ color: "var(--txt)" }}>{members.length}</strong> member{members.length !== 1 ? "s" : ""} in <strong style={{ color: "var(--txt)" }}>{selWS.workspacename}</strong></>
              : "Select a workspace to view its team"}
          </p>
        </div>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <select
            value={selWS?.work_id || ""}
            onChange={e => setSelWS(workspaces.find(w => w.work_id == e.target.value) || null)}
          ...
          {/* refresh */}
          {selWS && (
            <button onClick={load} title="Refresh members" style={{
              background: "var(--srf2)", border: "1.5px solid var(--bdr)", color: "var(--txt2)",
              padding: "8px 10px", borderRadius: 7, cursor: "pointer", display: "flex",
            }}>
              <Ico n="ref" s={16} />
            </button>
          )}

          {/* add member button — only visible when a workspace is selected */}
          {selWS && (
            <Btn v="primary" onClick={() => { setModal(true); setMsg(""); setForm({ username: "", role: "DEVELOPER" }); }}>
              <Ico n="plus" s={15} /> Add Member
            </Btn>
          )}
        </div>
      </div>
          ...
          : members.length === 0
            ? (
              <Empty
                ...
                sub="Invite your first teammate to this workspace."
                cta="Add Member"
                onCta={() => { setModal(true); setMsg(""); setForm({ username: "", role: "DEVELOPER" }); }}
              />
            )
            : (
              <Card style={{ padding: 0, overflow: "hidden" }}>
                ...
                <div style={{
                  ...
                  padding: "12px 22px", borderBottom: "1px solid var(--bdr)",
                  background: "var(--srf2)",
                }}>
                  {["Member", "Email / ID", "Role"].map(h => (
                    <span key={h} style={{ fontSize: 11.5, fontWeight: 700, color: "var(--txt2)", textTransform: "uppercase", letterSpacing: ".06em" }}>{h}</span>
                  ))}
                </div>
                ...
                {members.map((m, i) => {
                  ...
                  return (
                    <div key={i} style={{
                      ...
                      borderBottom: i < members.length - 1 ? "1px solid var(--bdr)" : "none",
                      transition: "background .15s",
                    }}
                      onMouseEnter={e => e.currentTarget.style.background = "var(--srf2)"}
                    ...
                    >
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        <div style={{
                          ...
                          display: "flex", alignItems: "center", justifyContent: "center",
                          fontFamily: "var(--font-d)", fontWeight: 700, fontSize: 15,
                          color: avatarBg === "var(--acc)" ? "var(--acc-fg)" : "#fff",
                        }}>
                          {uname[0].toUpperCase()}
                        </div>
      ...
      {/* ── Add Member Modal ── */}
      {modal && (
        <Modal title={`Add Member to "${selWS?.workspacename}"`} onClose={() => { setModal(false); setMsg(""); }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
            ...
            <div style={{
              ...
              Enter the <strong style={{ color: "var(--txt)" }}>exact username</strong> of the person you want to invite. They must already have a Jira account.
            </div>
            ...
            <Field label="Role">
              <select value={form.role} onChange={e => setForm(p => ({ ...p, role: e.target.value }))}>
                <option value="PROJECT_MANAGER">Project Manager — can create &amp; delete projects</option>
                <option value="DEVELOPER">Developer — can create &amp; update tasks</option>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {Object.entries(ROLE_MAP).map(([key, cfg]) => (
                <button key={key}
                  onClick={() => setForm(p => ({ ...p, role: key }))}
                  style={{
                    background: form.role === key ? cfg.bg : "var(--srf2)",
                    color: form.role === key ? cfg.c : "var(--txt2)",
                    border: `1.5px solid ${form.role === key ? cfg.c : "var(--bdr)"}`,
                    borderRadius: 7, padding: "5px 13px", fontSize: 12.5, fontWeight: 600,
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              <Btn v="secondary" onClick={() => { setModal(false); setMsg(""); }}>Cancel</Btn>
              <Btn v="primary" onClick={invite} disabled={saving}>
                {saving ? "Inviting…" : <><Ico n="users" s={15} /> Add Member</>}
              </Btn>
            </div>
          </div>
        </Modal>
